...
public class ImplementWatcherService implements Runnable{
	WatchService watchDir;
	PeerProcedure myPeer;
	public ImplementWatcherService(PeerProcedure myPeer, String fileDir) throws IOException{
		this.myPeer = myPeer;
	...
	private void triggerMsg(String fName) {
		for (FileConf f : myPeer.localFiles) {
    		if(f.fName.equals(fName)) {
    			if (myPeer.isPush) {
	    			f.incrementVersionNum();
	    			Thread threadInstance = new Thread(new NeighbourThread(myPeer.generateMessageId(), myPeer.getInfo.ipAddr, myPeer.getInfo.ID, myPeer.getInfo.portNum, fName.toString(), f.versionNum, "invalidate"));
